...
import { getFeedQueryFn, deleteUserPostMutationFn, getFollowingQueryFn, followUserMutationFn, unfollowUserMutationFn, getUserPostsQueryFn } from "@/lib/api";
...
const SocialMainPage = () => {
  ...
  const handleLike = (postId: string) => {
    if (!userId || likedPosts.includes(postId)) return;
            ...
            <div className="mt-8 flex flex-col gap-4">
              {loading ? (
                ...
                posts.map(post => {
                  ...
                  const dateStr = format(createdDate, "dd.MM.yyyy");
                    ...
                    <div key={post._id} id={`post-${post._id}`} className="p-4 border rounded bg-white relative">
                      ...
                      <div className="absolute right-2 top-2 z-10">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="p-1"><MoreHorizontal className="w-5 h-5" /></Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem asChild>
                              <Link to={`/u/users/${post.author.username}`}>Профиль</Link>
                            </DropdownMenuItem>
                            {isOwner && (
                              <DropdownMenuItem onClick={() => handleDeletePost(post._id)} className="text-red-500">Удалить</DropdownMenuItem>
                            )}
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                      <div className="flex items-center gap-4 mb-2">
                        ...
                        <div className="flex flex-col justify-center min-w-0">
                          <Link to={`/u/users/${post.author.username}`} className="font-bold text-lg truncate hover:underline">{post.author.name}</Link>
                          {/* Кнопка подписки только если не подписан и не владелец */}
                          {!isOwner && !isFollowing && (
                            <Button
                              size="sm"
                      ...
                      <div className="flex items-center gap-4 mt-4 border-t pt-2">
                        <button
                          className={`flex items-center gap-1 text-pink-500 ${likedPosts.includes(post._id) ? 'opacity-60 pointer-events-none' : 'hover:opacity-80'}`}
                          onClick={() => handleLike(post._id)}
                          disabled={likedPosts.includes(post._id) || !userId}
                        >
                          <Heart className="w-5 h-5" fill={post.likes?.length ? "#ec4899" : "none"} />
                          <span className="font-semibold text-base text-gray-700">{post.likes?.length || 0}</span>
                        </button>
                        <Button
                          variant="ghost"
                          size="sm"
