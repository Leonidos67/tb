...
export function MorphSurface({ onSubmit, autoOpen = false, hideDock = false }: { onSubmit?: (message: string) => void | Promise<void>; autoOpen?: boolean; hideDock?: boolean }) {
    ...
    >
      ...
      >
        <FooterContext.Provider value={context}>
          {!hideDock && <Dock />}
          <Feedback ref={feedbackRef} onSuccess={onFeedbackSuccess} onSubmitMessage={onSubmit} />
        ...
        <div className="flex w-fit items-center gap-2">
          <AnimatePresence mode="wait">
            {showFeedback ? (
              ...
              >
                <SiriOrb size="24px" colors={{ bg: "oklch(22.64% 0 0)" }} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
